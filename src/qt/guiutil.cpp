...
{
    // Convert whycoin:// to whycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("whycoin://"))
    {
        uri.replace(0, 11, "whycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WhyCoin.lnk";
}
...
{
    return GetAutostartDir() / "whycoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("WhyCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  whycoin-qt [" + tr("command-line options") + "]                     " + "\n";
