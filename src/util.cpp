...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WhyCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "whycoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "whycoind.pid"));
